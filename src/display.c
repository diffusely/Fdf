...
/*   Updated: 2025/04/09 23:33:05 by noavetis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	map_helper(int i, int j, t_view *v)
{
	...
	s = 20 * v->zoom;
	st.z = v->map->mt[i][j];
	st.x = iso_x(j * s, i * s) + v->map->point.x;
	st.y = iso_y(j * s, i * s, st.z) + v->map->point.y;
	if (j < v->map->width - 1)
	{
		end.x = iso_x((j + 1) * s, i * s) + v->map->point.x;
		end.y = iso_y((j + 1) * s, i * s,
				v->map->mt[i][j + 1]) + v->map->point.y;
		v->color = get_color((st.z + v->map->mt[i][j + 1]));
		draw_line(st, end, v);
	}
	if (i < v->map->height - 1)
	{
		end.x = iso_x(j * s, (i + 1) * s) + v->map->point.x;
		end.y = iso_y(j * s, (i + 1) * s,
				v->map->mt[i + 1][j]) + v->map->point.y;
